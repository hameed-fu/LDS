            ...
            @if ($attempts->count() > 0)
                <div class="card border-0 shadow-lg rounded-4 mb-5">
                                ...
                                <div class="accordion-item border-0 shadow-sm rounded-3 mb-3">
                                    <h2 class="accordion-header" id="heading{{ $index }}">
                                        <button class="accordion-button collapsed fw-semibold bg-light" type="button"
                                            ...
                                            Attempt #{{ $index + 1 }} —
                                            <span class="text-success ms-2">{{ $attempt->score }}%</span>
                                            <span class="ms-3 text-muted small">
                                                (Correct: {{ $attempt->correct_count }} | Wrong:
                                                {{ $attempt->wrong_count }})
                                            </span>
                                        </button>
                                    </h2>
                                    ...
                                    <div id="collapse{{ $index }}" class="accordion-collapse collapse"
                                        aria-labelledby="heading{{ $index }}" data-bs-parent="#attemptsAccordion">
                                        <div class="accordion-body">
                                            @foreach ($quiz->questions as $question)
                                                @php
                                                    $answer = $attempt->answers[$question->id] ?? null;
                                                    $selected = $answer['selected'] ?? null;
                                                    $correctIds = $answer['correct'] ?? [];
                                                @endphp

                                                <div class="mb-4 p-4 rounded border shadow-sm bg-white">
                                                    <h6 class="fw-semibold mb-3">{{ $loop->iteration }}.
                                                        {{ $question->question_text }}</h6>
                    ...
                    @else bg-light @endif">
                                                                ...
                                                                @if ($isCorrect)
                                                                    <span
                                                                        class="badge bg-success text-white fw-semibold">Correct</span>
                                                                @elseif($isSelected && !$isCorrect)
                                                                    <span
                                                                        class="badge bg-danger text-white fw-semibold">Your
                                                                        Answer</span>
                                                                @endif
                                                            </div>
                                                        @endforeach
            ...
            <form method="POST" action="{{ route('student.quiz.attempt', $quiz->id) }}" class="mb-5">
                ...
                <div class="card shadow-lg border-0 rounded-4 p-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4">
                        ...
                        <button type="submit" class="btn btn-lg text-white fw-semibold rounded-pill px-4 py-2 shadow-sm"
